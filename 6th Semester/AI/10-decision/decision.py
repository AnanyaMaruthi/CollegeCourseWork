...
def getEntropy(data):
    ...
    if len(contents) == 1:
        return 0
    entropy = -1 * (((contents[0]/total) * math.log2(contents[0]/total)) +  ((contents[1]/total) * math.log2(contents[1]/total)))
    return entropy
...
def calculateColumnEntropy(data, column):
    ...
    for attribute in attributes:
        try:
            current_entropy = getEntropy(data.query(column + " == '" + attribute + "'"))
        except:
            current_entropy = getEntropy(data.query(column + " == '" + str(int(attribute)) + "'"))
        entropy += attributeCount[attribute] / sum(attributeCount) * current_entropy
    return entropy
...
def getTree(data):
    ...
    if rootEntropy == 0:
        return set(data["Play"]) # change accordingly
    ...
    for attribute in set(data[root]):
        try:
            subTree[attribute] = getTree(data.query(root + "== '" + attribute + "'"))
        except:
            subTree[attribute] = getTree(data.query(root + "== '" + str(int(attribute)) + "'"))
